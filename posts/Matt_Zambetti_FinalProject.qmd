...
An interesting note about this plot is that the points seem to be evenly distributed for the range of salaries. Unline the expereince graph where we saw a dark band through the center, this one seems a bit more spread out.
...
ggplot(industry_jobs,aes(x=`Age`, y=`Salary`, color=Role)) +
  ...
  ggtitle("Age vs Salary") 
```
In this plot we can see that Developer, Engineer, Director and Manager all dominate the plot. But, what is also interesting and self explanitory is that most of the higher paying roles are Managerial Roles, where the lower paying jobs are developers. One job that follows the positive trend and seems to be evenly distributed is an engineer. It seems that this position is valued through ones career.
...
Going even deeper into each of these plots you notice small trends. For example, in both age and experience for the total distribution you see the trend of salaries plateauing after a certain age/number of years. I listed many possible reasons for this, but it would be interesting to see if the companies that provided these numbers would ellaborate on why these trends are present.

The other trend that I found very interesting, was that of the Age vs Salary plot for only industry jobs. If one looks closely to the darker regions of the graph, there is a staircase like behavior to it.
