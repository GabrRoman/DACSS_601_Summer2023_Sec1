...
lap_times %>%
  ...
  filter(laptime_seconds < 300, raceId >= 948, gp_name %in% c("Abu Dhabi Grand Prix", "Belgian Grand Prix", "Hungarian Grand Prix", "Monaco Grand Prix")) %>%
...
Let's start with race lap times. Since there are so many tracks, we'll just select a random set of 5 to look at, and we'll also remove outlier laps  over 150 seconds.
...
selected_laps %>%
  ...
  ggplot(aes(x = laptime_seconds, fill = post_rules_change)) + geom_density() + 
  facet_wrap(~gp_name) +
  scale_fill_manual(values = c("white", "orange"))
```
There is again no clear pattern here.
...
Next, we can look at qualifying times to see if we see the same pattern.


# QUALIFYING TIMES TO-DO
