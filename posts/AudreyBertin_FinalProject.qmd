...
We then plot the number of failures in 2021 vs 2022 for each type of mechanical failure to see if we can get a sense of overall trends:
...
mechanical_failures %>%
  ...
  geom_point( aes(x=status, y=`2021`, color="2021"), size=3 ) +
  geom_point( aes(x=status, y=`2022`, color="2022") , size=3 ) +
  coord_flip() +
  theme_minimal() +
  xlab(NULL) +
  ylab("# of Retirements") +
  ...
  theme_minimal() +
  labs(color = "Year") + 
  scale_color_manual(values = c("skyblue", "orange1"))
```
...
```
Let's start by looking at retirements. Specifically, at retirements that were the driver's fault, and not due to a mechanical issue with their car. This can give us a quick overview of the number of "mistakes" each driver is making throughout the season. 

Below are the unique statuses for drivers who retired from 2021-2022. 
