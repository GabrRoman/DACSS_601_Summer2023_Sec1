...
race_results <- results %>%
  ...
  inner_join(status, by = 'statusId') %>%
  select(code, surname, constructor_name, year, gp_name, start_position, finish_position, status, retired, points)
...
overall_results %>%
  ...
  ggplot(aes(x = surname, y = retirements, fill =year)) + geom_col() + facet_wrap(~constructor_name, scales = "free_x") + xlab(NULL) + ylab("# of Retirements") + ggtitle("# of Retirements By Driver and Year", subtitle = "For Driver Pairings That Raced in both 2021 and 2022")
```
