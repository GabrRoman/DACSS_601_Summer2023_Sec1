...
This very simple function that reads in the [SNL Cast Dataset](https://github.com/hhllcks/snldb) and converts the first and last episode date columns to date format^[Note that most values here are `NA`].  
...
Our function will compute the mean, median, and standard deviation for a variable. Specifically, the function will take a vector (or a column from a data frame) and output a tibble of these summary statistics.  
...
make_my_hist(hotels,lead_time,fill="blue")
```
