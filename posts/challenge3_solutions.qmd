...
Because the animal weights data is in tabular format, it is easy to see that $n=9$ regions (categories or cases) in the original data, and that there are $k=16$ types of livestock (categories or columns). Therefore, we expect the pivoted dataset to have $9 * 16$ = `r nrow(animal_weight)*(ncol(animal_weight)-1)` rows and 3 columns (region, animal type, and animal weight.)
