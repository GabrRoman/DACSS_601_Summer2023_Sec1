...
atlantic_hurricanes <- 
read.csv("_mysampledatasets/atlantic_hurricanes.csv")
...
- X (or the list number/ID of the hurricane as entered into the dataset, mostly used for organizational, data entry purposes)
...
wind_speed2 <- atlantic_hurricanes2

wind_speed2 <- separate(atlantic_hurricanes2, Wind.speed, into = c("Wind.speed.mph", "Wind.speed.kmh"), sep = "(")
...
```{r}
wind_speed2 %>% select(-REf, -X)
```

```{r}
wind_speed2 %>% select(-REf, -X)

pressure_2 <- wind_speed2

pressure_2 <- separate(wind_speed2, Pressure, into = c("Pressure.hPa", "Pressure.inHg"), sep = " ")
...
```{r}
pressure_2 %>% select(-REf, -X)
```

## What Remains to Be Cleaned
While I did remove the REf and X variables and separated the Wind.speed and Pressure columns into columns with distinct units of measurement (note: while all of this appears in R Studio, the code that I created to separate the wind speed units of measurements into separate columns is not being picked up in the course blog format when I attempt rendering this), I'm aware that there are additional items to be cleaned. 

Going forward, I want to keep the information within the Duration column but potentially rename it to Date and create a separate Duration column that transforms the date timeline/range for each hurricane into a number of days. I know some of the datasets we've used in class for the challenges have dealt with aspects of tidying date-related data (like separating or grouping months and years), but I'm not exactly sure yet on how to go about transforming a date range into number of days within R Studio without going through and making manual changes to each row myself. In addition, I still have to remove the phrase "mph" from the values in the Wind.speed.mph column, remove the parantheses and "km/h" from the Wind.speed.kmh values, remove the "hPa" phrase from the Pressure.hPa column, remove the parentheses and "inHg" from the Pressure.inHg column--so that R can classify those variables as numeric values that I can then use to compute summary statistics with. 
