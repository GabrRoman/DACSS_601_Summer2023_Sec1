...
All of the data types seem reasonable here so there is no need to modify them. 

There appear to be 77 unique circuits.

```{r}
length(unique(circuits$circuitId))
```
...
```{r message=FALSE}
constructos <- readr::read_csv("_data/formula1/constructors.csv")
```
...
Lap times has unique identifiers referring to the race and driver who completed them, which we can use to join tables later. We also have lap number, driver position, and lap times. The `time` variable is in an `hms` class format, which is appropriate for storing time. We don't need to modify any data types here.
